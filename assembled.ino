...
void generate_score_point()
{
  ...
  //check availability
  for (int n = 0; n < snake_length+1; n++)
  {
    //the point is on snake, regenerate
    if (pos(snake[n][0], snake[n][1]) == pos(column, row))
    {
  ...
  if (pos(snake[0][0]+1, snake[0][1]) == pos(column, row))
    {
      return generate_score_point();
    }
      if (pos(snake[0][0]-1, snake[0][1]) == pos(column, row))
    {
      ...
      if (pos(snake[0][0], snake[0][1]-1) == pos(column, row))
    {
      return generate_score_point();
    }
      if (pos(snake[0][0], snake[0][1]+1) == pos(column, row))
...
void move_snake()
{
  ...
  {
    //increase the snake_length to add the deleted tail
    snake_length++;
    //generate new score point
    generate_bonus();
    generate_score_point();
    //increase the snake_speed every time
  ...
  {
    //if the snake_length is over 3, decrease the snake_length by 1
    if (snake_length > 3)
    {
      //set the LED for tail off
      strip.setPixelColor(pos(snake[snake_length][0], snake[snake_length][1] + 8), strip.Color(0, 0, 0));
      snake_length--;
    }
...
void show_snake()
{
  ...
  //body
  for (int n = 1; n < snake_length - 1; n++)
  {
    strip.setPixelColor(pos(snake[n][0], snake[n][1] + 8), strip.Color(0, 255, 0));
  }
  //tail
  strip.setPixelColor(pos(snake[snake_length - 1][0], snake[snake_length - 1][1] + 8), strip.Color(0, 0, 255));
  strip.setPixelColor(pos(score_point[0], score_point[1]), strip.Color(255, 0, 0));
  strip.show();
}
